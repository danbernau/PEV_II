...
#line 289 "./crude1.w"
    ...
    for (beta = 0.01; beta <= 1.0; beta += 0.01) {
        S = 0.0;
        X = 0.0;
        V = 0.0;
        double a;
...
#line 319 "./crude1.w"
	...
	fprintf(fp, "\n%f, %6.16f, %6.16f,%6.16f", beta, Q, V,sqrt(V));
    }
...
pt_net Initialize(char*filename) {
    ...
    pt_net pt_n;
    FILE*fp;
    int i, j, node1, node2, num, distance;
    double reliability;
    ...
    for (i = 1; i <= pt_n->numnodes; i++) {
        ...
        for (j = 1; j <= num; j++) {
            fscanf(fp, "%d%lf%d", &node2, &reliability, &distance);
            pt_n->I[node1][node2].adj = 1;
            pt_n->I[node1][node2].rlb = reliability;
            pt_n->I[node1][node2].dst = distance;
        }
    }
...
int X(int node1, int node2, pt_net nt) {
    ...
    double epsilon;
    double modified_beta;
    epsilon = 1 - nt->I[node1][node2].rlb;
	/*
		Trying diferent improvements to the normal algorithm
			1st try:     modified_beta = (double) beta/nt->I[node1][node2].dst; 	
			2nd try:     modified_beta = (double) beta/(4 - nt->I[node1][node2].dst);		
			3rd try:     modified_beta = randn(beta, (double) beta/nt->I[node1][node2].dst);		 
			4th try:     modified_beta = randn(beta, (double) sqrt(pow(beta/nt->I[node1][node2].dst,2)));		
	*/

    modified_beta = (double) beta/(4 - nt->I[node1][node2].dst);
    if (U < modified_beta) {
        tempL = (1 - epsilon) / (1 -  modified_beta);
        l *= tempL;
        return 1;
    } else {
        tempL = epsilon /  modified_beta;
        l *= tempL;
        return 0;
    }
}
